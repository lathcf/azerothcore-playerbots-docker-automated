...
# LAST so it overrides the image defaults. Sized for this 32GB box, which SHARES RAM with a
# RAM-hungry worldserver (2000 bots). The mod-playerbots wiki's "buffer pool = 50% of RAM" rule assumes a
# DEDICATED DB host; on a shared box that risks OOM, so we size to the working set + headroom
# (8G) instead — still ~60x the stock 128M default. Only broadly-compatible innodb options are
# used (no innodb_use_fdatasync / skip-log-bin) so a bad option can't put the DB in a boot loop.
# If the DB ever fails to start, delete config/mysql-tuning.cnf and the mount and re-run.
mkdir -p "$AC_DIR/config"
cat > "$AC_DIR/config/mysql-tuning.cnf" <<'CNF'
# Generated by setup.sh — do not edit by hand. See setup.sh for sizing rationale.
[mysqld]
...
# Load so we can talk to the DB later with the right password.
set -a
...
# throughput knobs, separate from the gameplay rates above. Tuned for this host: 16 cores, 32GB
# RAM, ~2000 bots. Update if the LXC's hardware changes.
#   worldserver.conf:
#   - MapUpdate.Threads: stock is 1 — the single biggest CPU lever with this many bots. The wiki
#     caps its recommendation at 6 even on big boxes (diminishing returns vs. DB/network/bot-AI
#     threads competing for cores), so we use 6 rather than cores-2.
...
set_conf "MapUpdate.Threads"               "6"  "$WS_CONF"
...
#   playerbots.conf:
#   - BotActiveAlone + botActiveAloneSmartScale: the wiki's "Profile 1 (best for high bot counts)".
#     Only ~10% of bots run full AI when no real player is near, and SmartScale auto-throttles that
#     further if the tick gets heavy. This is what the comment on the population block assumed but
#     never actually set — now it's pinned.
#   - PlayerbotsDatabase.WorkerThreads/SynchThreads: the wiki's recommended bot-DB thread split.
#   - RandomBot*Interval: the module authors' tuned cadence for the random-bot manager.
set_conf "AiPlayerbot.BotActiveAlone"               "10"   "$PB_CONF"
set_conf "AiPlayerbot.botActiveAloneSmartScale"     "1"    "$PB_CONF"
set_conf "PlayerbotsDatabase.WorkerThreads"         "1"    "$PB_CONF"
set_conf "PlayerbotsDatabase.SynchThreads"          "2"    "$PB_CONF"
